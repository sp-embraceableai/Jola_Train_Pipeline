...
training_config = {
    ...
    "output_dir": './output_olmo2_0425_1b_sft_local'
}

# Use local ShareGPT dataset
data_config = {
    "train_size": 100,  # Reduced for testing
    "task_name": "sharegpt",
...
print(f"Dataset: {data_config['data_path']}")
...
# dataset setting - use ShareGPT dataset
print("Loading ShareGPT dataset...")
jola_dataset = ShareGPTDatasetFromFile(
    data_path=jola_config_dict["data_config"]["data_path"],
    train_size=jola_config_dict["data_config"]["train_size"],
    format_type="chatml"
)
jola_data = jola_dataset.data_from_file()

print(f"Dataset loaded successfully!")
print(f"Train samples: {len(jola_data['train'])}")
print(f"Validation samples: {len(jola_data['valid'])}")
